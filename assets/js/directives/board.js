zurvives.directive('board', function($http, boardData) {
  ...
  function link($scope, element, attrs) {
    ...
    io.socket.get('/api/map/create', function (result) {
      $scope.rendering = true;
      var boardGround = result.layer2d["Board"];
      ...
      function drawImage () {
        ...
        var layer2dLength = Object.keys(result.layer2d).length;

        // $.each(boardData.layer2d, function(name, tiles) {
        var layerData = result.layer2d["Board"];
      ...
      function initLayer(layerData, tilesetsheet, tileSize) {
        for (var y = 0; y < $scope.rowTileCount; y++) {
          for (var x = 0; x < $scope.colTileCount; x++) {
            ...
            $.each(result.layer2d, function(name, tiles) {
              if(name !== "Board") {
                var tile = tiles[x][y];
                if(tile !== 0) {
        ...
        io.socket.post('/games/play/mapLoaded', {}, function () {
        });
